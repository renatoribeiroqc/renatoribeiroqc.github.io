...
class SecurityService
{
    ...
    private $formTokenLabel = 'csrfmiddlewaretoken';
    ...
    public function insertHiddenTokenToObj()
    {
        $csrfToken = $this->getCSRFToken();
        return array("name" => $this->xssafe($this->formTokenLabel), "value" => $this->xssafe($csrfToken));
    }
    ...
    public static function getTokenByKey($key = null) {
        $keyArray = array(
            'markSession' => '',
            'setAgenda' => '',
            'reschedule' => ''
    ...
    public static function getToken($onlyToken = false) 
    {
        ...
        return '<input type="hidden" name="csrf_token" value="' . htmlspecialchars($_SESSION['token']) . '">';
    }

    public static function getFixedToken()
    {
        if (isset($_SESSION['fixedToken'])) {
            unset($_SESSION['fixedToken']);
        }

        $_SESSION['fixedToken'] = bin2hex(openssl_random_pseudo_bytes(32));

        return '<input type="hidden" name="csrf_token" value="' . htmlspecialchars($_SESSION['fixedToken']) . '">';
    }

    public static function validateToken($token, $method = null)
    {
        if (isset($_SESSION['token'])) {

            // Quando o token vier do form de pesquisa, a validação é feita com token fixo
            if ($method == "searchagenda") {
                if ($_SESSION['fixedToken'] == $token) {
                    return true;
                }
            }
